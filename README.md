...
**左栏（全高）**放身份与读数：宽档（≥ 120 列，40 列宽）画 fs 标记（5 行字符画，静止时亮品红→品红渐变，**fs-agent 干活时整块沿一条 6 帧的色环走**（只走亮色相，不带明暗交替 —— 那是「变色」不是「闪」），每帧 400ms、2.4 秒一圈，停下就回到渐变），窄档（80–119 列，28 列宽）退成一行 `fs-agent <版本>`，再窄就**整栏隐藏**、转录吃掉全部宽度。标记下面是 tab 条（`调用量` / `轨迹` / `文件`，**用鼠标点**切换，后两页还没做、写一句占位），再下面是六个读数（上下文 / token / 回合 / 输入 / 输出 / 缓存）。**去留由宽度决定，内容由高度决定**：高度不够时先丢标记、再丢身份行、最后从尾部丢字段，上下文 / token / 回合这三行最后才走。
