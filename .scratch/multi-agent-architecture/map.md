...
> **交棒已发生（2026-09-26 补记）**：这份决策集已经折成 **[`.scratch/fs-agent-v1/spec.md`](../fs-agent-v1/spec.md)**（问题陈述 + 用户故事 + 20 节实现决定 + 测试决定 + Out of Scope），`/to-tickets` 在它下面切出 **32 张实现票、全部 `done`**。本图至此**只作决策存档**：实现阶段暴露的问题回到那份 spec 与对应实现票去改，**不要再往这张图加票**。
