...
- **2026-09-23 修正（用户评审）**：工具行不再印参数摘要，改成**一句描述**：
  ...
  - **同一个产出函数**（`wording::tool_call_line(tool, args)`）：转录的折叠行与**权限询问弹窗里
    的描述行**都由它产出，所以「你回答的问题」与「它问的那一行」不会各说各话。弹窗里描述行
    **下面还留着要执行的原文**（`bash（command=…）`）——授权正是唯一必须看得见确切命令的时刻
    （2026-09-23，用户选择「描述 + 原文都要」）。
  - **2026-09-23 再次收窄（用户指出弹窗自己重复）**：标题**不再带工具名**（只留 `权限询问：`），
    **「一句白话」那行整行删除**（`permission_summary` 随之从措辞层移除，连同它的测试；
    `custom_tool_parts` 只服务于它，也一并删）。弹窗正文从四行变三行：
    `权限询问：` / `调用 工具 描述` / 要执行的原文 —— 工具名与「要做什么」都只出现一次。
...
- **2026-09-23 修正（用户评审，两处）**：
  - 中段覆盖层的**候选键行居中**：按钮行按**自己的宽度**居中（原先它的起点是从正文那段
    居中文字的居中量推出来的，正文那行很宽时按钮就被留在左边）。命中区随字形一起移动。
  - 详情覆盖层的**底部计数是「屏幕上最后一行」**，不是「窗口第一行」：滚到底就该读到
    `↕ 154/154`（原先读 `↕ 94/154`）；内容短于窗口时读 `↕ m/m`。
