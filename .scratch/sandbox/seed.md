...
| 怎么选到它 | `config.toml` 的 `[permissions] mode = "workspace"` 作默认 + `--mode workspace` 旗标（今天模式是硬编码 `Ask`，三处组装点：`src/cli.rs:369/663/1557`）；运行期不加切换手势（`Shift+Tab` 仍归 plan） |
| 工作区内的操作 | 一律允许（今天项目**内**的 `.env` 家族 / `.git` / `.ssh` / shell rc 文件 / 断路器短路**照旧硬拒**，不放宽） |
| 工作区外的操作 | **读与写都要问**（今天两者都被硬拒：`SessionPaths` 在解析期拒掉，且那是规则无法下压的 deny floor，`tests/permission_gate.rs::the_path_limit_is_a_deny_floor`）。注意这与 DSH 不同——那边读是随便的，我们更严 |
| 工作区外的 shell | **未定**，见下 |
...
1. **一律问** —— 与我们今天 `ask` 档的现状**等同**（`src/permissions.rs:118-127`：`(Ask, 非 ReadOnly) → Ask`），包成新模式只是改名，不带来任何「区外」语义；
...
2. **边界：只限写还是读写都限**：DSH 是整机只读 + 工作区可写。我们今天的 cwd 限制比它严（读写都限）。沙箱做出来后，「读」要不要一起放开？这直接决定上面那条「读也问」的决议还算不算数。
