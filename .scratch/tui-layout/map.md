...
- **实现票 10–16**（`Status: ready-for-agent`，就在本目录 `issues/` 下，编号接在设计票之后）：10 全屏骨架（生命周期 / 几何 / 降级 / 提示行 / 管线 / 删 inline 路径）、11 对话面板（滚动 / 缓存 / 吸底 / 鼠标）、12 输入编辑器（`Input` / 多行 / 粘贴 / 两个确认）、13 信息面板、14 模态覆盖层、15 CLI 斜杠命令、16 测试与验证迁移。
