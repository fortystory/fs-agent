...
**提示符色相（PromptHue）**:
输入区提示符 `❱ ` 的颜色：**界面里唯一在动的东西**。色相每秒走 0.3 圈（约 3.3 秒一轮），同时饱和度以 0.55 为中心、幅度 0.2、每秒 3.0 弧度地呼吸，明度固定 0.85；**24 位真彩**（`Color::Rgb`），因为色相走 16 色的名字会看出台阶。它是**纯渲染器状态**，不进事件流；参数是从用户自用的一条脚本搬来的（`.scratch/tui-input-pulse/spec.md` §2b）。
_Avoid_: 光标色（那是终端的事）、主题色、语法高亮色

**脉冲（Pulse）**:
那个帧计数器本身：`TuiState.pulse`，由循环里**常开**的定时器推进（`PULSE_FRAME` 60 ms 一帧），**从不归零**；今天的消费者是**提示符色相**。它是**纯渲染器状态**，不进事件流。**空闲时那台时钟也在跑** —— 被驱动的东西（提示符）在等待输入时就在屏幕上；这是对「空闲没有任何定时器」那条旧决议的正式反转（票 08），代价是空闲时每 60 ms 醒一次、最多重画两个格子。
