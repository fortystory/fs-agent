...
**脉冲（Pulse）**:
那个帧计数器本身：`TuiState.pulse`，由循环在**一次运行在飞时**才 arm 的定时器推进（`PULSE_FRAME` 60 ms 一帧），运行结束**归零**；今天的消费者是**提示符色相**。它是**纯渲染器状态**，不进事件流。**空闲时那台时钟不存在** —— 没有唤醒、没有重画（票 09 把票 08 的「常开」收了回去：颜色在用户打字时继续动是噪音，不是生命）。
