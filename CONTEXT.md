...
**转动短横（TurningDash）**:
一次运行在飞时 `fs-agent` 里那个短横的四个朝向，顺时针走 `─ → ╲ → │ → ╱`（`wording::DASH_TURN`），`PULSE_FRAME` 250 ms 一个朝向、1 秒转完一圈，停下就静止。它是界面里**唯一**的「正在工作」信号，两处身份都戴它：宽档是 mark 里那一格（4 格宽的横线），窄档是文字身份行 `fs-agent 0.1.0` 里那一格。用 box-drawing 而不是 ASCII `- \ | /`：对角是真对角、四个字形同一条基线，转起来不抖。
