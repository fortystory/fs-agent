...
**下落短横（FallingDash）**:
一次运行在飞时 `fs-agent` 里那个短横**往下落**：形状始终是那条 `▀▀▀▀`，只有所在的行在变 —— 宽档走 mark 自己的五行（`PULSE_FRAME` 250 ms 一行、一落 1.25 秒，落到底再从最上面出现），窄档是一行里能表达的三个高度 `▀ ▀ █ ▄ ▄`（`wording::DASH_FALL`，与 mark 同一套帧）。它是界面里**唯一**的「正在工作」信号，两处身份都戴它；**空闲时仍是 mark 原来那一行**（第 3 行、`logo_lines` 自己那四列），逐字节不变 —— 只在忙碌时出现的动画不许改掉静止的样子（票 06 的教训：旋转、以及换字形，都被否掉了）。
_Avoid_: spinner（那是命令行语汇）、loading 动画、进度（它只讲「还在跑」，不讲跑到哪了）

**忙碌脉冲（Pulse）**:
那个帧计数器本身：`TuiState.pulse`，只在一次运行在飞时由循环的定时器推进（`PULSE_FRAME` 250 ms 一帧），运行结束归零；今天的消费者是**下落短横**落在哪一行。它是**纯渲染器状态**，不进事件流；计时器**只在忙碌时 arm**，空闲时那台时钟根本不存在。颜色环（`PULSE_PALETTE`）已经**退出屏幕**，代码留着 —— 两版真机都读作「闪」或「突然」，不要再把它接回渲染路径。
