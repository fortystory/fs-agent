...
**转动短横（TurningDash）**:
一次运行在飞时 `fs-agent` 里那个短横的四个朝向，顺时针走 `─ → ╲ → │ → ╱`（`wording::DASH_TURN`），`PULSE_FRAME` 250 ms 一个朝向、1 秒转完一圈，停下就静止。它是界面里**唯一**的「正在工作」信号，两处身份都戴它，但**各用自己那一族的字形**：宽档是 mark 里那一格，四个朝向全用块字形（`▀▀▀▀` → `▚` 斜阶梯 → `█` 立柱 → `▞` 斜阶梯），**空闲时就是 mark 原来的 `▀▀▀▀`** —— 只在忙碌时出现的动画不许顺手改掉静止的样子；窄档是文字身份行 `fs-agent 0.1.0` 里那一格，用 box-drawing `─ ╲ │ ╱`（文字行里没有块格子可用，那里的对角得是真对角，也不用 ASCII `- / | \` 那种会抖的）。
_Avoid_: spinner（那是命令行语汇）、loading 动画、进度（它只讲「还在跑」，不讲跑到哪了）

**忙碌脉冲（Pulse）**:
那个帧计数器本身：`TuiState.pulse`，只在一次运行在飞时由循环的定时器推进（`PULSE_FRAME` 250 ms 一帧），运行结束归零；今天的消费者是**转动短横**的朝向。它是**纯渲染器状态**，不进事件流；计时器**只在忙碌时 arm**，空闲时那台时钟根本不存在。颜色环（`PULSE_PALETTE`）已经**退出屏幕**，代码留着 —— 两版真机都读作「闪」或「突然」，不要再把它接回渲染路径。
